...
class DatasetAbstractRepository(abc.ABC):
    @abc.abstractmethod
    def get_by_id(self, access_token, dataset_id, version_id):
        return NotImplementedError

    @abc.abstractmethod
    def save_dataset(self, access_token, version_id, dataset_object):
        return NotImplementedError
...
class DataGalaxyDatasetRepository(DatasetAbstractRepository):
    def __init__(self):
        integration_token = os.environ.get("DATAGALAXY_INTEGRATION_TOKEN", '')
    ...
    def _get_access_token(self, integration_token):
